...
public class CalculationsUptoDate {
...

	{
		...
		// lsp
		MasterEmployeeDetails edate = masterEmployeeDetailsRepo.findById(employeeId).get();
		LocalDate jd = edate.getJoiningDate();
		InternalExpenses totalDays=null;
		try {
		 totalDays = internalExpenseRepo.findByEmployeeById(employeeId).get();
		 }
		catch (Exception e) {
		...
		if (edate.getStatus() == EmployeeStatus.EXIT || edate.getStatus() == EmployeeStatus.TERMINATED
				|| edate.getStatus() == EmployeeStatus.ABSCONDED)
			
			endingDate = releaseEmpDetailsRepository.findByMasterEmployeeDetailsId(edate).get().getReleaseDate();			
		
		else
			endingDate = LocalDate.now();
			

		totalDays.setTotalDays(ChronoUnit.DAYS.between(jd, endingDate));
		internalExpenseRepo.save(totalDays);
		}
		...
		if (salaryRecords == 1)// number_of_salary_records_of_single_employee==1
		{
...

			{
				...
				Date2 = releaseEmpDetailsRepository.findByMasterEmployeeDetailsId(MasterEmployee).get().getReleaseDate();
...
//	}
	...
	public double bench_pay(List<Salary> salarylist, MasterEmployeeDetails MasterEmployee) {
			...
			if (!clientlist.isEmpty()) {
				...
				for (EmployeesAtClientsDetails client : clientlist) {
...

					{
						...
						} else if ((pos.isAfter(Date1) || pos.isEqual(Date1))
								&& (pos.isBefore(Date2) || pos.isEqual(Date2))
		...
		return Math.ceil(totalBenchpay);
